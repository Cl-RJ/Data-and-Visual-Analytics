...
class HW2_sql():
    ...
    def part_1_b_movies(self,connection,path):
        ...
        with open(path, 'r', encoding='latin-1') as file:
            ...
            for movie in movies:
                connection.execute("INSERT INTO movies (id, title, score) VALUES (?, ?, ?)", 
    ...
    def part_3(self,connection):
        ...
        part_3_sql = """
        SELECT printf('%.2f', 
            CAST(COUNT(CASE WHEN m.score >= 7 AND m.score <= 20 THEN 1 END) AS FLOAT) * 100.0 / 
            CAST(COUNT(*) AS FLOAT)
        ) 
        FROM movies m
        """
    ...
    def part_5(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_5_sql = """
        SELECT m.title, printf('%.2f', m.score), COUNT(mc.cast_id) as cast_count
        FROM movies m
        INNER JOIN movie_cast mc ON m.id = mc.movie_id
        GROUP BY m.id, m.title, m.score
        ORDER BY m.score DESC, cast_count ASC, m.title ASC
        LIMIT 5;
        """
        ######################################################################
    ...
    def part_6(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_6_sql = """
        SELECT cast_id, cast_name, printf('%.2f', AVG(m.score)) as avg_score
        FROM movie_cast mc
        INNER JOIN movies m ON mc.movie_id = m.id
        WHERE m.score >= 25
        GROUP BY cast_id, cast_name
        HAVING COUNT(*) >= 3
        ORDER BY AVG(m.score) DESC, cast_name ASC
        LIMIT 10;
        """
        ######################################################################
    ...
    def part_7(self,connection):
        ...
        SELECT DISTINCT 
            ...
            printf('%.2f', AVG(m.score)) as average_movie_score
        FROM movie_cast mc1
        ...
        HAVING COUNT(DISTINCT mc1.movie_id) >= 2 AND AVG(m.score) >= 40;
        """
        ######################################################################
    ...
    def part_8(self,connection):
        ############### EDIT SQL STATEMENT ###################################
        part_8_sql = """
        WITH CollaborationScores AS (
            -- Get scores for cast_member_id1
            SELECT cast_member_id1 as cast_id, average_movie_score
            FROM good_collaboration
            UNION ALL
            -- Get scores for cast_member_id2
        ...
        SELECT 
            cs.cast_id,
            mc.cast_name,
            printf('%.2f', AVG(CAST(cs.average_movie_score AS FLOAT))) as collaboration_score
        FROM CollaborationScores cs
        JOIN movie_cast mc ON cs.cast_id = mc.cast_id
        GROUP BY cs.cast_id, mc.cast_name
        ORDER BY collaboration_score DESC, mc.cast_name ASC
        LIMIT 5;
        """
        ######################################################################
    ...
    def part_9_a(self,connection,path):
        ############### EDIT SQL STATEMENT ###################################
        connection.execute("DROP TABLE IF EXISTS movie_overview;")
        part_9_a_sql = """
        ...
        ############### CREATE IMPORT CODE BELOW ############################
        with open(path, 'r', encoding='latin-1') as file:
            overviews = csv.reader(file)
            for overview in overviews:
                connection.execute("INSERT INTO movie_overview(id, overview) VALUES (?,?)", 
    ...
    def part_9_c(self,connection):
        ...
        SELECT COUNT(*)
        FROM movie_overview
        WHERE overview MATCH 'space NEAR/5 ship';
        """
        ######################################################################
