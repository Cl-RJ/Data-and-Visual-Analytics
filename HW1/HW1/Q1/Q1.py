...
class Graph:
    ...
    def add_edge(self, source: str, target: str) -> None:
        ...
        if source == target:
            return
        if not any((e[0] == source and e[1] == target) or 
                   (e[0] == target and e[1] == source) 
                   for e in self.edges):
    ...
    def max_degree_nodes(self) -> dict:
        ...
        for node in self.nodes:
            node_id = node[0]
            degree = sum(1 for edge in self.edges if node_id in edge)
            degrees[node_id] = degree

        max_degree = max(degrees.values())

        return {node_id: degree for node_id, degree in degrees.items() 
...
class  TMDBAPIUtils:
    ...
    def get_movie_cast(self, movie_id:str, limit:int=None, exclude_ids:list[int]=None) -> list:
        ...
        cast = data['cast']
        # Sort by order and filter by limit if specified
        cast = sorted(cast, key=lambda x: x.get('order', float('inf')))
        if limit is not None:
            cast = [c for c in cast if c.get('order', float('inf')) < limit]
        
        # Exclude specified IDs
        cast = [c for c in cast if c.get('id') not in exclude_ids]
    ...
    def get_movie_credits_for_person(self, person_id: str, start_date: str = None, end_date: str = None) -> list:
        ...
        if start_date:
            credits = [c for c in credits if c.get('release_date', '') >= start_date]
        if end_date:
            credits = [c for c in credits if c.get('release_date', '') <= end_date]
...
if __name__ == "__main__":
    # Initialize graph with Laurence Fishburne as starting node
    graph = Graph()
    graph.add_node(id='2975', name='Laurence Fishburne')
    tmdb_api_utils = TMDBAPIUtils(api_key='0387fa4280bbd30676b936bf92e9a1ea')

    base_actor_movies = tmdb_api_utils.get_movie_credits_for_person('2975', 
                                                                   start_date='1999-01-01', 
                                                                   end_date='1999-12-31')
    new_nodes = set()
    
    for movie in base_actor_movies:
        cast = tmdb_api_utils.get_movie_cast(movie['id'], limit=5)
        for actor in cast:
            actor_id = str(actor['id'])
            if actor_id != '2975':  # Skip Fishburne himself
                graph.add_node(actor_id, actor['name'].replace(',', ''))
                graph.add_edge('2975', actor_id)
                new_nodes.add(actor_id)
    ...
    for _ in range(2):
        ...
        for actor_id in current_nodes:
            movies = tmdb_api_utils.get_movie_credits_for_person(actor_id, 
                                                               start_date='1999-01-01', 
                                                               end_date='1999-12-31')
            ...
            for movie in movies:
                cast = tmdb_api_utils.get_movie_cast(movie['id'], limit=5)
                for costar in cast:
                    costar_id = str(costar['id'])
                    if costar_id not in new_nodes:
                        graph.add_node(costar_id, costar['name'].replace(',', ''))
                        graph.add_edge(actor_id, costar_id)
                        new_nodes.add(costar_id)
